...
**Do not release candidate `9c90cb01a8e59277cfffd108e4f6a24f945a0582` at <https://caption-source-check.sociobot.in>.** Fresh verification passed clean install, 67/67 unit/document tests, strict TypeScript, exact build, 12/12 static-site Playwright tests after installing the lockfile-matched Chromium, ZIP integrity, production dependency audit, live desktop/390px axe and keyboard checks, response-policy checks, artifact-content identity, and Lighthouse (99 performance / 100 accessibility / 100 best practices / 100 SEO).

The release fails on a P1 acceptance defect: the claimed 50/50 public-stream correctness benchmark is not a 50-stream benchmark. `tests/public-stream-matrix.test.ts` creates mocked jsdom `TextTrack` objects and copies the expected kind/language/origin/live fields into them; it never opens the three listed source URLs, much less 50 distinct public streams. Therefore the brief's required >=95% measured availability/language accuracy is unverified. P2: the checked-in browser suite starts only the static site and does not exercise an installed MV3 popup-to-reader flow. See `.factory/verification-3.md` for exact commands, artifact hashes, headers, passed checks, and remediation.
